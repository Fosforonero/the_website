...
excerpt: "A local translation pipeline: English JSON → Python → Ollama (qwen2.5:7b on Apple Silicon) → Next.js, generating 1,243 multilingual SEO pages for FitMesh Sync. API cost: zero. The code, the choices, the honest limits."
date: "2026-06-22"
tag: "Dev"
image: "/blog/ollama/traduzione-locale-ollama-qwen.webp"
imageAlt: "Illustration of the local translation pipeline with Ollama: a traduci.py Python script running a qwen model, 1,200 pages translated at zero cost for FitMesh Sync's multilingual site"
---
...
One thing up front, because it matters: this isn't taking work away from a translator. It replaces a paid API in a specific case (a zero-budget indie app) where the realistic alternative was never "pay a professional", it was "don't translate at all". For text where language really matters, a human translator is a different category, and a 7B model doesn't come close.
...
The rule about em-dashes and typographic quotes looks like nitpicking. It isn't. Models love "pretty" typographic characters (the long dash instead of a plain hyphen, curly quotes instead of straight ones), which then break the JSON, the alignment in the `.ts` files, and the snippets in meta tags. Saying it explicitly in the prompt costs one line and removed an entire class of downstream errors.
...
2. **A translation cache.** Today a copy update re-translates everything. With a hash of the source string as the key, I'd re-translate only what changed, and updates would go from minutes to seconds. **Update (July 2026):** I built it. It became [loctron](/en/blog/build-your-own-translation-engine), our translation engine with memory, a glossary and a DeepL plus Ollama waterfall, and it now translates the Flutter app too.
...
That translation cache I mentioned above, meanwhile, grew into a real engine, with memory, a glossary and multiple engines, able to translate the app too: [why we built our own Weglot](/en/blog/build-your-own-translation-engine).

> A 7B model on a laptop isn't GPT-4, and isn't trying to be. For a zero-budget indie project the question isn't what the best translation is in the abstract, but how to ship in 11 languages without the cost stopping you. This is one workable answer, limits and all.
