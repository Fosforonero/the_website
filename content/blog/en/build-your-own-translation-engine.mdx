---
title: "How I built my in-house translation engine"
...
A year ago I wrote about [how I translated 1,200 pages into 11 languages with Ollama for €0](/en/blog/translating-1200-pages-ollama). It was a Python pipeline that fed raw JSON into a local model and produced static pages for [FitMesh Sync](https://www.fitmesh.fit). It worked, but at the end of that article I left a list of three things I would do differently. The first line of that list read: "a translation cache, with a hash of the source string as the key, so I only re-translate what changed".
...
The obvious path was [Weglot](https://www.weglot.com/) or an equivalent like [ConveyThis](https://www.conveythis.com/). They are serious products, they do their job, and for the right site they are an excellent choice. For me they were the wrong choice, for four concrete reasons.
...
Simple [ICU](https://unicode-org.github.io/icu/userguide/format_parse/messages/) placeholders like `{count}` get masked; the complex ones with plurals or selects I leave to manual handling, because their structure is too fragile to hand to a machine.
...
This detail is central: **I never hardcode the free plan limit**, I read it from the API. DeepL's [free plan](https://developers.deepl.com/docs/resources/usage-limits) gives 500,000 characters per month, but what matters for the code is the real value the endpoint reports, month by month.
...
| Memory ownership | Inside their service | In my repo, versioned |
