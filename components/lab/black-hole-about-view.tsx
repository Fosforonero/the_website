...
const COPY: Record<Locale, Copy> = {
  it: {
    ...
    sections: [
      ...
      },
      {
        ...
        eqs: [
          ...
          { label: "Deflessione di campo debole fuori dal guscio (camera → ∞)", tex: "\\alpha(t_0\\!\\to\\!\\infty) = \\frac{2M}{b}\\left(1 - \\frac{t_0}{\\sqrt{b^{2}+t_0^{2}}}\\right) \\;\\xrightarrow[\\;t_0\\to-\\infty\\;]{}\\; \\frac{4M}{b}\\ \\text{(Einstein 1916)}" },
    ...
    refs: [
      ...
      { cite: "M. Janssen et al. (2025), «Deep learning inference with the Event Horizon Telescope I–III», A&A 698, A60–A62: reti neurali bayesiane sui dati EHT. Per Sgr A* indicano uno spin quasi massimale (a ≈ 0,8–0,9, prograda) e un disco quasi frontale, con emissione dominata da elettroni caldi del disco più che da un getto, coerenti con i preset di Kerr che usiamo.", url: "https://doi.org/10.1051/0004-6361/202553786" },
      { cite: "M. Gorski & L. Murchikova (2026), «The Discovery of an Active Wind from the Milky Way's Central Black Hole», ApJL: prima evidenza (ALMA + Chandra) di un vento attivo da Sgr A*, una cavità conica ~1 pc svuotata dal gas freddo. Copertura INAF.", url: "https://www.media.inaf.it/2026/06/08/vento-sagittarius-a-star/" },
      { cite: "NASA/Goddard Space Flight Center (2024), «New NASA Black Hole Visualization Takes Viewers Beyond the Brink»: per un buco nero da 4,3 milioni di M☉ (la massa di Sagittarius A*), orizzonte degli eventi di ~25 milioni di km (~17% della distanza Terra-Sole); una volta oltrepassato, la spaghettificazione arriva 12,8 secondi dopo, dopo aver percorso ~128.000 km verso la singolarità.", url: "https://science.nasa.gov/universe/black-holes/supermassive-black-holes/new-nasa-black-hole-visualization-takes-viewers-beyond-the-brink/" },
      { cite: "INAF (2026), «Un buco nero vicino per capire il passato lontano»: SDSS J110546 ospita un buco nero di massa relativamente piccola ma in rapida crescita per accrescimento, con emissione radio persistente da oltre 8 anni e un getto relativistico di recente formazione — un esempio osservativo reale del fenomeno che il toggle «Getti» di questa simulazione rende in forma stilizzata.", url: "https://www.media.inaf.it/2026/07/07/radio-burst-sdss-j110546/" },
      { cite: "M. Toscani et al. (2025), «Updated predictions for gravitational wave emission from tidal disruption events…», A&A 703, A75: le onde gravitazionali dagli eventi di disruzione mareale, il fenomeno riprodotto nel playground.", url: "https://doi.org/10.1051/0004-6361/202555648" },
      { cite: "Stack: Three.js, React Three Fiber, @react-three/drei, @react-three/postprocessing, KaTeX. Esiste anche un renderer WebGPU nativo (shader WGSL, stessa fisica) alla pagina /lab/buco-nero/webgpu. Sviluppo: Fosforonero, Matteo Pizzi (Roma)." },
    ],
  ...
  en: {
    ...
    sections: [
      ...
      },
      {
        ...
        body: [
          ...
          "The same mechanism inscribes its signature on the background star field, not just the disk. Every background star in fact has infinitely many distinct images: the primary (rays that never orbit), the secondary (rays that orbit once), the tertiary, and so on. The boundary between successive orders is a genuine mathematical caustic — a ray just outside does not orbit, a ray just inside orbits exactly once — and produces sharp circular rings in the star field, nested around the shadow with a spacing that shrinks by e^{−π} ≈ 1/23 per order. These rings are not a graphical artifact: they appear in every correct GR ray tracer, including the DNGR code that produced Gargantua for Interstellar (James et al. 2015). If you see them in the render, it means the integrator is correctly resolving the caustic structure of the lensing.",
        ],
        eqs: [
      ...
      },
      {
        ...
        body: [
          ...
          "In WebGPU mode the Playground runs 300,000 particles on exact prograde Kerr orbits with small epicyclic perturbations (radial epicyclic frequency κ, the same that vanishes at the ISCO): the accretion disk is live, not a painted image. The «Ringdown» button simulates a perturbation: the photon ring (the returning radiation — rays that have made at least one loop around the hole) pulses with the damped sinusoid A(t) = exp(−ω_I·t)·cos(ω_R·t), calibrated to tabulated Kerr l=2 quasi-normal mode frequencies (Leaver 1985). The quality factor Q = ω_R/(2ω_I) = 2.1 + 11.3·a³ is physically correct: at zero spin the hole rings ~2 times; near a≈0.99 it rings ~13 times before settling — the same signature LIGO measures in the tail of every black-hole merger. Vary the spin and watch the decay change.",
        ],
        eqs: [
      ...
      },
      {
        ...
        body: [
          "The per-ray step budget is finite: to avoid spending it just reaching the hole (which made the disk and shadow glitch when zooming far out), rays starting beyond the influence sphere R_far are advanced analytically in a straight line (spacetime there is essentially flat) and the geodesic march only begins where curvature matters. That is why the view holds even at large distance. Rays that never enter the influence sphere do not sample a «straight» sky, though: they receive Einstein's analytic weak-field deflection — the angle α = 4M/b, weighted for the camera-to-infinity stretch only — so the lensing decays smoothly with growing impact parameter instead of switching off at the sphere's rim, which otherwise showed in the sky as a large circular seam. Near the photon sphere, by contrast, the step is refined to resolve returning radiation and the sub-rings; and inside the 3D volumetric disk's thickness the step is bound to the local scale height H(r), in proportion to the ray's vertical slope, so the hydrostatic Gaussian is sampled densely enough not to render as discrete slabs (banding) — while plane-skimming rays, which see no vertical variation, keep the long step and never exhaust the budget.",
        ...
        eqs: [
          ...
          { label: "Weak-field deflection outside the shell (camera → ∞)", tex: "\\alpha(t_0\\!\\to\\!\\infty) = \\frac{2M}{b}\\left(1 - \\frac{t_0}{\\sqrt{b^{2}+t_0^{2}}}\\right) \\;\\xrightarrow[\\;t_0\\to-\\infty\\;]{}\\; \\frac{4M}{b}\\ \\text{(Einstein 1916)}" },
      ...
      },
      {
        ...
        body: [
          ...
          "Adaptive quality, one render per device. The same shader must run on an entry-level phone and on a discrete-GPU workstation, and WebGL has no vendor-specific code (one GLSL for everyone), so the real lever is dosing the load. The «Auto» mode detects the GPU (via WEBGL_debug_renderer_info: NVIDIA/Radeon/Apple-Silicon on desktop, Adreno/Mali/Apple on mobile) and picks a profile: on high-end desktop the Yoshida-6 integrator, 2× supersampling and the volumetric disk; on mobile the cheaper Euler integrator with more steps and a capped DPR, because there the bottleneck is fill-rate, not steps. Since some browsers mask the GPU for privacy, a governor measures the FPS (exponential moving average) and dynamically scales the load: on mobile the render resolution first (the bottleneck is fill-rate), then — only if that is not enough — the step count; on desktop the steps. Resizing the canvas is itself a small hitch, though, so the governor is anti-oscillation: a resolution that dropped the FPS becomes a ceiling it never climbs back to (otherwise vsync, quantising FPS to 60↔30, would bounce it between the two thresholds forever), recovery waits out a cooldown, and the FPS average restarts after every adjustment so the resize hitch does not pollute the next measurement. The heavy modes (Ultra, supersampling, 3D disk) stay isolated behind GLSL #defines compiled only when needed, so the default mobile shader stays small and compiles everywhere.",
    ...
    refs: [
      ...
      { cite: "M. Janssen et al. (2025), “Deep learning inference with the Event Horizon Telescope I–III”, A&A 698, A60–A62: Bayesian neural networks on EHT data. For Sgr A* they point to a near-maximal spin (a ≈ 0.8–0.9, prograde) and a near face-on disk, with emission dominated by hot disk electrons rather than a jet, consistent with the Kerr presets we use.", url: "https://doi.org/10.1051/0004-6361/202553786" },
      { cite: "M. Gorski & L. Murchikova (2026), “The Discovery of an Active Wind from the Milky Way's Central Black Hole”, ApJL: first evidence (ALMA + Chandra) of an active wind from Sgr A*, a ~1 pc conical cavity cleared of cold gas. INAF coverage.", url: "https://www.media.inaf.it/2026/06/08/vento-sagittarius-a-star/" },
      { cite: "NASA/Goddard Space Flight Center (2024), “New NASA Black Hole Visualization Takes Viewers Beyond the Brink”: for a 4.3-million-M☉ black hole (Sagittarius A*'s mass), an event horizon of about 25 million km (~17% of the Earth–Sun distance); once crossed, spaghettification arrives 12.8 seconds later, after covering ~128,000 km toward the singularity.", url: "https://science.nasa.gov/universe/black-holes/supermassive-black-holes/new-nasa-black-hole-visualization-takes-viewers-beyond-the-brink/" },
      { cite: "INAF (2026), “A nearby black hole to understand the distant past”: SDSS J110546 hosts a relatively low-mass but rapidly growing black hole, with persistent radio emission for over 8 years and a newly formed relativistic jet — a real observational example of the phenomenon this simulation's stylized “Jets” toggle represents.", url: "https://www.media.inaf.it/2026/07/07/radio-burst-sdss-j110546/" },
      { cite: "M. Toscani et al. (2025), “Updated predictions for gravitational wave emission from tidal disruption events…”, A&A 703, A75: gravitational waves from tidal disruption events, the phenomenon reproduced in the playground.", url: "https://doi.org/10.1051/0004-6361/202555648" },
      { cite: "Stack: Three.js, React Three Fiber, @react-three/drei, @react-three/postprocessing, KaTeX. A native WebGPU renderer (WGSL shader, same physics) also exists at /en/lab/black-hole/webgpu. Development: Fosforonero, Matteo Pizzi (Rome, Italy)." },
    ],
